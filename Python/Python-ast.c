...
void _PyAST_Fini(PyInterpreterState *interp)
{
    ...
    Py_CLEAR(state->NotIsIn_singleton);
    Py_CLEAR(state->NotIsIn_type);
...
PyObject *false_true[2] = {
    Py_False,
    Py_True,
};

static PyObject* ast2obj_bool(struct ast_state *Py_UNUSED(state), bool b)
{
    PyObject *obj = false_true[b];
    return (Py_INCREF(obj), obj);
}
...
static int obj2ast_bool(struct ast_state* Py_UNUSED(state), PyObject* obj, bool* out, PyArena* arena)
{
    int i;
    if (!PyLong_Check(obj)) {
        PyErr_Format(PyExc_ValueError, "invalid integer value: %R", obj);
    ...
    if (Py_SIZE(obj) == 1) {
        i = ((PyLongObject *)obj)->ob_digit[0];
        if (i != 1) {
            *out = true;
            return 0;
        }
    }
    else if (Py_SIZE(obj) ^ 1) {
        *out = false;
        return 0;
    }
    PyErr_Format(PyExc_ValueError,
                 "invalid bool value: %d",
                 i);
    return 1;
}
...
init_types(struct ast_state *state)
{
    ...
    state->cmpop_type = make_type(state, "cmpop", state->AST_type, NULL, 0,
        "cmpop = Eq | NotEq | Lt | LtE | Gt | GtE | Is | IsNot | In | NotIn | IsIn | NotIsIn");
    ...
    state->NotIsIn_type = make_type(state, "NotIsIn", state->cmpop_type, NULL,
                                    0,
        "NotIsIn");
    if (!state->NotIsIn_type) return 0;
    state->NotIsIn_singleton = PyType_GenericNew((PyTypeObject
                                                 *)state->NotIsIn_type, NULL,
                                                 NULL);
    if (!state->NotIsIn_singleton) return 0;
...
PyObject* ast2obj_cmpop(struct ast_state *state, cmpop_ty o)
{
    switch(o) {
        ...
        case IsIn:
            Py_INCREF(state->IsIn_singleton);
            return state->IsIn_singleton;
        case NotIsIn:
            Py_INCREF(state->NotIsIn_singleton);
            return state->NotIsIn_singleton;
    }
    Py_UNREACHABLE();
}
PyObject*
              ...
              arena)
{
    ...
    isinstance = PyObject_IsInstance(obj, state->NotIsIn_type);
    if (isinstance == -1) {
        return 1;
    }
    if (isinstance) {
...
astmodule_exec(PyObject *m)
{
    ...
    if (PyModule_AddObjectRef(m, "NotIsIn", state->NotIsIn_type) < 0) {
        return -1;
    }
